...
def propagate_terms(terms_df, subontologies):
    ...
    return pd.DataFrame(propagated_terms), ancestor_lookup
...
if __name__ == '__main__':
    ...
    # Remove terms not in subontologies
    bpo_terms = set(subontologies['BPO'].nodes)
    obsolete_terms = set.difference(set(annotation_df['term']), bpo_terms)
    print(obsolete_terms)
    annotation_df = annotation_df[annotation_df['term'].isin(bpo_terms)]
    
    if args.prop:
        print('Propagating Terms')
        (annotation_df, ancestor) = propagate_terms(annotation_df, subontologies)

    annotation_df.to_csv('propagated_annotations.tsv', sep='\t', index=False, header=True)
