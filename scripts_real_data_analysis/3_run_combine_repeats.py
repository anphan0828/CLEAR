...
with open('main_result.txt', 'a') as f:
    for each in run_list:
        ...
        combined_table, higher_is_better = combine_repeats(input_files)
        if higher_is_better:
            ties = combined_table.groupby('value').apply(lambda x: len(x)).sort_index(ascending=False).to_dict()
            table_score = combined_table[['term','value']].rename(columns={"value":"score"})
        else:
            ties = combined_table.groupby('pvalue').apply(lambda x: len(x)).sort_index(ascending=True).to_dict()
            table_score = combined_table[['term','pvalue']].rename(columns={"pvalue":"score"})
