...
def average_overlap_coefficient(table, annotation_dict):
    ...
    return np.mean(overlap_coefficients) if overlap_coefficients else 0.0
...
def calculate_from_input(input,annotations,num):
    ...
    score_overlap = average_overlap_coefficient(chosen_score, dict_T)
    res = {"score_overlap": round(score_overlap,4)}
    return res
        
        
def parse_inputs(argv):
    parser = argparse.ArgumentParser(
        description='Compute overlap coefficient of top n terms using p-value ranking or posterior probability')
